...
- Public Article API/cache contract
- GitHub OAuth-backed community comments/questions MVP
- Node-owned TTS and article Q&A public routes backed by the internal Python worker contract
- Node→Python internal worker contract for future TTS/Q&A/RAG
- lint/test/build/check scripts

`api.seojing.com`의 공개 경계는 Node/Fastify 서비스가 소유합니다. Python 구현은 TTS/Q&A/RAG 같은 장기 작업을 위한 loopback-only worker dependency이며, Cloudflare Tunnel이나 공개 reverse proxy에 직접 노출하지 않습니다.
...
```bash
# TTS/Q&A/RAG worker integration is disabled unless explicitly enabled.
PYTHON_WORKER_ENABLED=false
PYTHON_WORKER_BASE_URL=http://127.0.0.1:4037
TTS_AUDIO_ROOT=.seojing-worker/tts-audio
curl http://localhost:4000/health/ready
```

Node→Python 내부 호출 계약과 운영 기준은 [`docs/python-worker-contract.md`](docs/python-worker-contract.md)에 정리했습니다. 공개 노출 포트는 Node API만 두고 Python worker는 loopback 전용으로 둡니다.

Public API surfaces currently include:

- `GET /articles/:slug` and related article metadata/body routes
- `POST /articles/:slug/qa` for source-backed article Q&A
- `GET /tts/summary`, `POST /tts/jobs`, `GET /tts/jobs/:jobId`, `GET /tts/audio/:jobId`
- `/community/*` GitHub-authenticated comments/questions routes
